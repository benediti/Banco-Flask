...
if base_fixa_file is not None and pagamentos_file is not None:
    try:
        ...
        # Pré-visualização dos arquivos para diagnóstico
        st.write("Pré-visualização da Base de Funcionários (Fixa):")
        st.dataframe(base_fixa.head())
        st.write("Pré-visualização do Arquivo de Pagamentos:")
        st.dataframe(pag.head())
